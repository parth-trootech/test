...
def login(email, password):
    payload = {"user_email": email, "user_password": password}  # Changed to use email instead of username
    response = requests.post(f"{BASE_URL}/login", json=payload)
    if response.status_code == 200:
        st.session_state.logged_in = True  # Store login state in session state
        st.session_state.user_email = email  # Store user_email in session state
        st.session_state.user_id = response.json().get("user_id")  # Store user_id
        st.session_state.page = "upload"  # Set page to image upload page
    else:
        st.error("Error: " + response.json().get("detail", "Invalid credentials"))
...
def image_upload_page():
    ...
    if image is not None:
        ...
        if st.button("Upload Image"):
            # Send image and user_id to backend
            files = {"image": image.getvalue()}  # Read the image content correctly as bytes
            response = requests.post(
                f"{BASE_URL}/upload_image",
                data={"user_id": st.session_state.user_id},  # Send user_id as data
                files={"image": ("image", files["image"], image.type)}  # Ensure you send proper MIME type
            )
            if response.status_code == 200:
                st.success("Image uploaded successfully!")
                image_id = response.json().get("image_id")
...
# Prediction Page
def prediction_page():
    st.title(f"Prediction for {st.session_state.user_email}")
    response = requests.post(f"{BASE_URL}/predict", json={"image_id": st.session_state.image_id})
    if response.status_code == 200:
        result = response.json()
        st.write(f"Predicted Digit: {result['predicted_digit']}")
